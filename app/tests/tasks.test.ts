...
import {createTasks,addTask,updateTask,toggleTask,removeTask,pending,dailyReset} from '../src/tasks.ts';
...
test('missed dailies are counted once per day, never on first launch',()=>{
 ...
 const fresh=createTasks({list:[{id:'a',text:'Lire',type:'daily',done:false}]} as any,day(3));assert.equal(fresh.lastReset,day(3,0));
});
test('corrupt saved data is sanitised',()=>{
 const s=createTasks({list:[{id:'a',text:'ok',category:'zzz',type:'weird'},{text:'no id'},null,{id:'b',text:''}]} as any);
