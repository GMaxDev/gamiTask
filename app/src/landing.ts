...
$('#lp-note-form').onsubmit=(e:Event)=>{e.preventDefault();const input=$('#lp-note') as HTMLInputElement,text=cleanText(input.value);if(!text)return;
  notes.unshift({id:crypto.randomUUID(),userId:'landing',text,note:'',kind:'todo',difficulty:'easy',value:0,done:false,createdAt:Date.now(),category:null,up:true,down:false,countUp:0,countDown:0,days:127,streak:0,dueAt:null,checklist:[],completedAt:null,focusCount:0});input.value='';renderNotes();};
