...
export function buildHat(p:Primitives,id:string,parent:any):any{
  ...
  switch(id){
    ...
    case 'hat-halo':{const ring=p.mesh(new THREE.TorusGeometry(.22,.03,8,24),p.mat(C.gold,{emissive:C.gold,emissiveIntensity:.7}),0,.82,0,g);ring.rotation.x=Math.PI/2;ring.castShadow=false;g.userData.float=true;break;}
