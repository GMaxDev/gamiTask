...
export function createPrimitives(getRoot:()=>any,materials:Map<string,any>,extras:any[]=[]):Primitives{
  ...
  function box(w:number,h:number,d:number,color:any,x:number,y:number,z:number,r=.04,parent?:any):any{const rr=r?Math.min(r,w/3,h/3,d/3):0;return mesh(shape(`b:${w},${h},${d},${rr}`,()=>rr?new RoundedBoxGeometry(w,h,d,2,rr):new THREE.BoxGeometry(w,h,d)),color,x,y,z,parent??getRoot());}
