...
export interface Progress{coins:number;xp:number;level:number;streak:number;achievements:string[];energy:number;exhausted:boolean}
const KEYS=new Set(ACHIEVEMENTS.map(a=>a.key));
export function createProgress():Progress{return {coins:0,xp:0,level:0,streak:0,achievements:[],energy:50,exhausted:false};}
...
export function setXp(p:Progress,u:{xp:number;level:number}):void{p.xp=u.xp;p.level=u.level;}
