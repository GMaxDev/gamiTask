...
export const levelOf=xp=>Math.floor(Math.sqrt(Math.max(0,xp)/50));
export const xpForLevel=level=>level*level*50;
const num=(v,fallback=0)=>Number.isFinite(v)&&v>=0?Math.floor(v):fallback;
...
export function levelInfo(p){const level=levelOf(p.xp),floor=xpForLevel(level),next=xpForLevel(level+1);return {level,into:p.xp-floor,span:next-floor,next};}
