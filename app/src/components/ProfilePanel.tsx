...
export function ProfilePanel({ data, isOwnProfile, onClose }: Props) {
  ...
  const xpPct =
    data.xpToNext > 0
      ? Math.floor((data.xpProgress / data.xpToNext) * 100)
      : 100;
  const hatEmoji = data.hat
    ? (SHOP_ITEMS.find((s) => s.id === data.hat)?.emoji ?? null)
    : null;

  return (
    <div id="profile-overlay" onClick={onClose}>
      <div id="profile-panel" onClick={(e) => e.stopPropagation()}>
        <button id="profile-close-btn" onClick={onClose} aria-label="Fermer">
          ✕
        </button>
        ...
        <div className="profile-section">
          <div className="profile-section-title">
            Succès ({data.achievements.length}/{ACHIEVEMENTS_DEF.length})
          </div>
